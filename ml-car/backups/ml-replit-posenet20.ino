...
void loop() {
 ...
 // All commands from webpage
        ...
        if (myIncoming == 'T'){  digitalWrite(D5, 1); digitalWrite(D6,0);  myDriveSpeed = myNormalDriveSpeed; delay(500); digitalWrite(D5, 0); digitalWrite(D6,1); while(client.read() >= 0);     }   // special command should go in reverse
        if (myIncoming == 'Y'){ myEmergencyStop = false; myDriveSpeed = 0; myTurnAmount = myTurnStraight; myTurnSpeed = 0;}   // this must be on website
