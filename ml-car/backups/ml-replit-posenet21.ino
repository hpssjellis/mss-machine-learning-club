...
// These are fancy feactures so be careful
...
char server[] = "ml-websocket-05--jerteach.repl.co"; // Node server needs to allow users to have read access to your url
...
//////////////////////////////////// global variables /////////////////////////////// 
 ...
 int myDriveSpeed = 0;
 int myMinDriveSpeed = 30;   // minimum drive speed unless stopped
 int myNormalDriveSpeed = 50;
 int myMaxDriveSpeed = 90;
...
void loop() {
 ...
 // All commands from webpage
        ...
        if (myIncoming == 'F'){ digitalWrite(D5, 0); digitalWrite(D6,1); if (myRegularDebugOn){ Particle.publish("D5-0,D6-1","backward",60,PRIVATE);} }
        if (myIncoming == 'G'){ digitalWrite(D5, 1); digitalWrite(D6,0); if (myRegularDebugOn){Particle.publish("D5-1,D6-0","forward",60,PRIVATE); } }
        ...
        if (myIncoming == 'Z'){ myEmergencyStop = true; }   // ermergency stop
        if (myIncoming == 'Y'){ myEmergencyStop = false; digitalWrite(D5, 1); digitalWrite(D6,0);   myDriveSpeed = 0; myTurnAmount = myTurnStraight; myTurnSpeed = 0;}   // this must be on website
        
           
        
       // if (myIncoming == 'T'){  digitalWrite(D5, 1); digitalWrite(D6,0); delay(10); myDriveSpeed = myNormalDriveSpeed; myTurnAmount = myTurnStraight; myTurnSpeed = myTurnNormalSpeed; delay(1000); digitalWrite(D5, 0); digitalWrite(D6,1); while(client.read() >= 0);     }   // special command should go in reverse
        if (myIncoming == 'T'){  digitalWrite(D5, 0); digitalWrite(D6,1);   myDriveSpeed = myNormalDriveSpeed; myTurnAmount = myTurnStraight; myTurnSpeed = myTurnNormalSpeed;  }   // special command should go in reverse
